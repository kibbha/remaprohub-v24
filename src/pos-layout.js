...
export function layoutButtonItem(button,catalog=[]){
  ...
  return item?{id:'layout:'+String(button?.id||''),name:item.name,price:item.price,tax_rate:item.taxRate,sku:item.sku,production_station:item.station,type:item.type,source:'layout',autoMatched:false}:null;
}
...
export function renderPosLayoutEditor({layout,catalog=[],published=null,history=[],selectedButtonId='',selectedPageId=''}) {
...
export function bindPosLayoutEditor(root,{getLayout,setLayout,getSelected,setSelected,getPage,setPage,catalog,onDirty,onSave,onPublish,onRestore,onSeed,onRender}){
  if(!root)return;
  ...
  root.querySelector('#posLayoutButtonForm')?.addEventListener('submit',e=>{
    ...
    mutate(doc=>{
      ...
      doc.buttons.push({
        ...
        availability:normalizeAvailability({mode:String(d.get('availabilityMode')||'unlimited'),manualQuantity:Number(d.get('manualQuantity'))||0,lowThreshold:Number(d.get('lowThreshold'))||3}),
  ...
  bindValue('#posLayoutColor','color',String);bindValue('#posLayoutW','w',Number);bindValue('#posLayoutH','h',Number);bindValue('#posLayoutStation','station',String);bindValue('#posLayoutFavorite','favorite',Boolean);bindValue('#posLayoutHidden','hidden',Boolean);bindValue('#posLayoutUnavailable','unavailable',Boolean);
  ...
  root.querySelector('#posLayoutAvailabilityMode')?.addEventListener('change',e=>mutate(doc=>{const b=doc.buttons.find(x=>x.id===getSelected());if(b)b.availability=normalizeAvailability({...b.availability,mode:e.target.value})}));
  root.querySelector('#posLayoutAvailabilityQty')?.addEventListener('change',e=>mutate(doc=>{const b=doc.buttons.find(x=>x.id===getSelected());if(b)b.availability=normalizeAvailability({...b.availability,manualQuantity:e.target.value})}));

...
export function normalizePosLayout(input){
  ...
  doc.pages=doc.pages.map((x,i)=>({id:String(x.id||uid('page')),name:String(x.name||'Page '+(i+1)),sortOrder:Number(x.sortOrder)||i,color:String(x.color||'#efe5d7')}));
  doc.categories=doc.categories.map((x,i)=>({id:String(x.id||uid('cat')),name:String(x.name||'Catégorie'),parentId:String(x.parentId||''),sortOrder:Number(x.sortOrder)||i,color:String(x.color||'#d9c4a7')}));
  ...
  doc.modifierGroups=doc.modifierGroups.map((g,i)=>({
    ...
    required:!!g.required,min:Math.max(0,Number(g.min)||0),max:Math.max(0,Number(g.max)||1),
...
export function seedPosLayoutFromCatalog(catalog=[]){
  ...
  doc.buttons=catalog.filter(x=>x.active!==false).map((item,i)=>({
    ...
    color:'#d6b98c',hidden:false,unavailable:false,favorite:i<4,station:String(item.production_station||''),
    modifierGroupIds:[]
  }));
...
export function renderPosLayoutEditor({layout,catalog=[],published=null,selectedButtonId=''}) {
  const doc=normalizePosLayout(layout),page=doc.pages[0]||null;
  ...
  const pageOptions=doc.pages.map(p=>'<option value="'+esc(p.id)+'">'+esc(p.name)+'</option>').join('');
  const catOptions=doc.categories.map(c=>'<option value="'+esc(c.id)+'">'+esc(c.parentId?'↳ ':'')+esc(c.name)+'</option>').join('');
...
export function bindPosLayoutEditor(root,{getLayout,setLayout,getSelected,setSelected,catalog,onDirty,onSave,onPublish,onSeed}){
  ...
  root.querySelector('#posLayoutPageForm')?.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(e.currentTarget);mutate(doc=>doc.pages.push({id:uid('page'),name:String(d.get('name')||'Page'),sortOrder:doc.pages.length,color:'#efe5d7'}));});
  ...
  root.querySelector('#posLayoutButtonForm')?.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(e.currentTarget),p=(catalog||[]).find(x=>String(x.id)===String(d.get('productId')));if(!p)return;mutate(doc=>{const i=doc.buttons.length,id=uid('btn');doc.buttons.push({id,pageId:String(d.get('pageId')||doc.pages[0]?.id||''),categoryId:String(d.get('categoryId')||''),productId:String(p.id),label:String(p.name||''),sortOrder:i,x:i%4,y:Math.floor(i/4),w:1,h:1,color:'#d6b98c',hidden:false,unavailable:false,favorite:false,station:String(p.production_station||''),modifierGroupIds:[]});setSelected(id)});});
  root.querySelectorAll('[data-layout-button]').forEach(el=>{
    el.addEventListener('click',()=>{setSelected(el.dataset.layoutButton);onDirty?.(false)});
    el.addEventListener('dragstart',e=>e.dataTransfer?.setData('text/plain',el.dataset.layoutButton));
    el.addEventListener('dragover',e=>e.preventDefault());
    el.addEventListener('drop',e=>{e.preventDefault();const from=e.dataTransfer?.getData('text/plain'),to=el.dataset.layoutButton;if(!from||from===to)return;mutate(doc=>{const sorted=[...doc.buttons].sort((a,b)=>a.sortOrder-b.sortOrder),a=sorted.findIndex(x=>x.id===from),b=sorted.findIndex(x=>x.id===to);if(a<0||b<0)return;const [m]=sorted.splice(a,1);sorted.splice(b,0,m);sorted.forEach((x,i)=>{x.sortOrder=i;x.x=i%4;x.y=Math.floor(i/4)});doc.buttons=sorted})});
  });
  const selected=()=>normalizePosLayout(getLayout()).buttons.find(x=>x.id===getSelected());
  ...
  root.querySelector('#posLayoutDeleteButton')?.addEventListener('click',()=>mutate(doc=>{doc.buttons=doc.buttons.filter(x=>x.id!==getSelected());setSelected(doc.buttons[0]?.id||'')}));
  root.querySelector('#posLayoutModifierForm')?.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(e.currentTarget);mutate(doc=>doc.modifierGroups.push({id:uid('mod'),name:String(d.get('name')||'Modificateurs'),type:String(d.get('type')||'supplement'),required:d.get('required')==='on',min:Number(d.get('min'))||0,max:Math.max(1,Number(d.get('max'))||1),station:'',options:[]}))});
  ...
  root.querySelectorAll('[data-layout-add-choice]').forEach(el=>el.addEventListener('click',()=>{const name=prompt('Nom du choix');if(!name?.trim())return;const required=confirm('Ce choix est-il obligatoire ?');const minRaw=prompt('Minimum',required?'1':'0');if(minRaw===null)return;const maxRaw=prompt('Maximum','1');if(maxRaw===null)return;mutate(doc=>{const m=doc.menus.find(x=>x.id===el.dataset.layoutAddChoice);if(m)m.choices.push({id:uid('choice'),name:name.trim(),required,min:Math.max(0,Number(minRaw)||0),max:Math.max(1,Number(maxRaw)||1),categoryId:'',productIds:[],modifierGroupIds:[]})})}));
}

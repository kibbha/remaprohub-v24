...
export function bindPosLayoutEditor(root,{getLayout,setLayout,getSelected,setSelected,getPage,setPage,setCategory,catalog,onDirty,onSave,onPublish,onRestore,onSeed,onRender}){
  ...
  root.querySelectorAll('[data-layout-page-name]').forEach(el=>el.addEventListener('change',()=>{const name=String(el.value||'').trim();if(!name)return;mutate(doc=>{const p=doc.pages.find(x=>x.id===el.dataset.layoutPageName);if(p)p.name=name})}));
  root.querySelectorAll('[data-layout-category-name]').forEach(el=>el.addEventListener('change',()=>{const name=String(el.value||'').trim();if(!name)return;mutate(doc=>{const category=doc.categories.find(x=>x.id===el.dataset.layoutCategoryName);if(category)category.name=name})}));
  ...
  root.querySelector('#posLayoutItemPrice')?.addEventListener('change',e=>{const value=Number(e.target.value);if(!Number.isFinite(value)||value<0){e.target.value='';return}mutate(doc=>{const b=doc.buttons.find(x=>x.id===getSelected());if(b?.item)b.item.price=Math.round(value*100)/100})});
  root.querySelector('#posLayoutItemTax')?.addEventListener('change',e=>{const value=Number(e.target.value);if(!Number.isFinite(value)||value<0||value>100){e.target.value='';return}mutate(doc=>{const b=doc.buttons.find(x=>x.id===getSelected());if(b?.item)b.item.taxRate=value})});
  ...
  root.querySelectorAll('[data-layout-option-form]').forEach(form=>form.addEventListener('submit',e=>{
    e.preventDefault();const d=new FormData(form),name=String(d.get('name')||'').trim(),priceDelta=Number(d.get('priceDelta'));if(!name||!Number.isFinite(priceDelta))return;
    mutate(doc=>{const g=doc.modifierGroups.find(x=>x.id===form.dataset.layoutOptionForm);if(g)g.options.push({id:uid('opt'),name,priceDelta:Math.round(priceDelta*100)/100,station:['kitchen','bar','none'].includes(String(d.get('station')))?String(d.get('station')):'',ingredientId:'',omitIngredient:g.type==='without'})});
  }));
  root.querySelectorAll('[data-layout-option-edit]').forEach(form=>form.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(form),[groupId,optionId]=String(form.dataset.layoutOptionEdit||'').split(':'),name=String(d.get('name')||'').trim(),priceDelta=Number(d.get('priceDelta'));if(!name||!Number.isFinite(priceDelta))return;mutate(doc=>{const option=doc.modifierGroups.find(x=>x.id===groupId)?.options.find(x=>x.id===optionId);if(!option)return;option.name=name;option.priceDelta=Math.round(priceDelta*100)/100;option.station=['kitchen','bar','none'].includes(String(d.get('station')))?String(d.get('station')):''})}));
  ...
  root.querySelector('#posLayoutMenuForm')?.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(e.currentTarget),name=String(d.get('name')||'').trim(),price=Number(d.get('price'));if(!name||!Number.isFinite(price)||price<0)return;mutate(doc=>doc.menus.push({id:uid('menu'),name,productId:String(d.get('productId')||''),price:Math.round(price*100)/100,choices:[]}))});
  root.querySelectorAll('[data-layout-menu-edit]').forEach(form=>form.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(form),name=String(d.get('name')||'').trim(),price=Number(d.get('price'));if(!name||!Number.isFinite(price)||price<0)return;mutate(doc=>{const m=doc.menus.find(x=>x.id===form.dataset.layoutMenuEdit);if(!m)return;m.name=name;m.productId=String(d.get('productId')||'');m.price=Math.round(price*100)/100})}));
  ...
  root.querySelectorAll('[data-layout-choice-edit]').forEach(form=>form.addEventListener('submit',e=>{e.preventDefault();const d=new FormData(form),[menuId,choiceId]=String(form.dataset.layoutChoiceEdit||'').split(':'),name=String(d.get('name')||'').trim(),min=Math.max(0,Number(d.get('min'))||0),max=Math.max(1,Number(d.get('max'))||1);if(!name||max<min)return;mutate(doc=>{const choice=doc.menus.find(x=>x.id===menuId)?.choices.find(x=>x.id===choiceId);if(!choice)return;choice.name=name;choice.required=d.get('required')==='on';choice.min=min;choice.max=max;choice.categoryId=String(d.get('categoryId')||'');choice.productIds=d.getAll('productIds').map(String).filter(Boolean)})}));

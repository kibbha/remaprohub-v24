...
function topbar(){
  ...
  return `<header class="topbar">
    ...
    <details class="pos-more-menu"><summary aria-label="Plus d’options">•••</summary><div class="pos-more-panel">
      ...
      <div class="pos-more-nav"><button class="nav-tab ${state.view==='directOrders'?'active':''}" id="nav-direct-orders">${t('directOrders')}${pending?' <span class="nav-badge">'+pending+'</span>':''}</button><button class="nav-tab ${state.view==='tickets'?'active':''}" id="nav-tickets">Tickets</button><button class="nav-tab ${state.view==='report'?'active':''}" id="nav-report">Rapport</button><button class="nav-tab ${state.view==='terminals'?'active':''}" id="nav-terminals">Terminaux</button><button class="nav-tab ${state.view==='printers'?'active':''}" id="nav-printers">Imprimantes</button><button class="nav-tab ${state.view==='team'?'active':''}" id="nav-team">Équipe</button></div>
...
function productGlyph(product={}){
  const text=((product.category||'')+' '+(product.name||'')).toLocaleLowerCase();
  if(/burger|sandwich/.test(text))return'🍔';
  if(/pizza/.test(text))return'🍕';
  if(/salade|bowl/.test(text))return'🥗';
  if(/dessert|gâteau|tiramisu|chocolat/.test(text))return'🍰';
  if(/boisson|coca|jus|eau|bière|vin|cocktail/.test(text))return'🥤';
  if(/café|coffee|thé|tea/.test(text))return'☕';
  if(/entrée|starter/.test(text))return'🥟';
  return'🍽️';
}
function mainView(){

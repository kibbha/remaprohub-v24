...
function posBrandLockup({auth=false,version=false}={}){
...
function topbar(){
  ...
  return `<header class="topbar">
    ...
    <details class="pos-more-menu"><summary aria-label="Plus d’options">•••</summary><div class="pos-more-panel">
      ...
      <button class="secondary" id="refresh-catalog" ${!state.online?'disabled':''}>Rafraîchir le catalogue</button>
      <button class="secondary" id="close-session" ${state.cashSession?.status!=='open'?'disabled':''}>Clôturer la caisse</button>
    </div></details>
  </header>`;
}
function floorView(){
...
function productPhotoUrl(product={},buttonPhoto=''){
  const src=String(buttonPhoto||product.photo||product.photo_url||product.photoUrl||product.image_url||product.imageUrl||product.image||'').trim();
  return /^(https?:\/\/|data:image\/(?:jpeg|png|webp);base64,)/i.test(src)?src:'';
}
function productVisual(product={},buttonPhoto='',badge=''){
  const src=productPhotoUrl(product,buttonPhoto);
  return '<span class="product-visual '+(src?'has-photo':'no-photo')+'">'+(src?'<img src="'+esc(src)+'" alt="" loading="lazy">':'<span class="product-visual-placeholder" aria-hidden="true"></span>')+badge+'</span>';
}
function mainView(){
  const catalog=state.bootstrap?.catalog||[],layout=activeLayout(),search=String(state.productSearch||'').trim().toLocaleLowerCase();
  let productArea='',categoryArea='';
  const matchesSearch=(label,category='')=>!search||(String(label||'')+' '+String(category||'')).toLocaleLowerCase().includes(search);
  if(layout?.document?.buttons?.length){
    ...
    const subcategoryArea=navigation.subcategories.length?'<nav class="layout-subcategory-tabs" aria-label="Sous-catégories">'+navigation.subcategories.map(c=>'<button type="button" aria-pressed="'+(String(c.id)===String(state.layoutCategoryId)?'true':'false')+'" class="'+(String(c.id)===String(state.layoutCategoryId)?'active':'')+'" data-layout-category="'+esc(c.id)+'">'+esc(c.name)+'</button>').join('')+'</nav>':'';
    categoryArea='<nav class="categories layout-categories"><div class="category-heading">Catalogue</div>'+navigation.roots.map(c=>'<button type="button" aria-pressed="'+(String(c.id)===navigation.activeRootId?'true':'false')+'" class="category '+(String(c.id)===navigation.activeRootId?'active':'')+'" data-layout-category="'+esc(c.id)+'">'+esc(c.name)+'</button>').join('')+'</nav>';
    productArea='<section class="products layout-products"><div class="layout-page-tabs">'+pages.map(p=>'<button type="button" aria-pressed="'+(String(p.id)===String(state.layoutPageId)?'true':'false')+'" class="'+(String(p.id)===String(state.layoutPageId)?'active':'')+'" data-layout-page="'+esc(p.id)+'">'+esc(p.name)+'</button>').join('')+'</div>'+subcategoryArea+'<div class="product-toolbar"><div><strong>'+buttons.length+' article'+(buttons.length>1?'s':'')+'</strong><small> · implantation v'+Number(layout.version||0)+(state.online?'':' · cache offline')+'</small></div><button class="secondary" id="quick-item">+ Article libre</button></div>'
      +(buttons.length?'<div class="layout-product-grid">'+buttons.map(b=>{const p=itemForButton(b,catalog);if(!p)return'';const linkedMenu=!p.layoutStandalone?(doc.menus||[]).find(m=>String(m.productId)===String(p.id)):null,a=availabilityForButton(b,catalog),remaining=a.availableNow,badge=remaining==null?'':remaining<=0?'<span class="availability-badge soldout">Épuisé</span>':'<span class="availability-badge '+(remaining<=a.lowThreshold?'low':'ok')+'">'+remaining+' dispo</span>',searchText=esc(((b.label||p.name)+' '+(p.category||'')).toLocaleLowerCase());return '<button type="button" class="product layout-product '+(b.unavailable||a.soldOut?'unavailable':'')+'" aria-label="'+esc((b.label||p.name)+' · '+money(linkedMenu?.price||p.price))+'" data-product-search="'+searchText+'" data-layout-product="'+esc(b.id)+'" '+(b.unavailable||a.soldOut?'disabled':'')+' style="--pos-color:'+esc(b.color||'#f2e5d8')+';--pos-x:'+(Number(b.x)||0)+';--pos-y:'+(Number(b.y)||0)+';--pos-w:'+Math.max(1,Number(b.w)||1)+';--pos-h:'+Math.max(1,Number(b.h)||1)+'">'+productVisual(p,b.photo,badge)+'<span class="product-copy"><strong>'+esc(b.label||p.name)+'</strong><small>'+esc(p.category||b.station||p.production_station||'')+'</small><span class="price">'+money(linkedMenu?.price||p.price)+'</span></span></button>'}).join('')+'</div>':'<div class="empty"><h3>Aucun article</h3><p>Changez de catégorie ou modifiez votre recherche.</p></div>')+'</section>';
  }else{
    ...
    categoryArea='<nav class="categories"><div class="category-heading">Catalogue</div>'+cats.map(c=>'<button type="button" aria-pressed="'+(c===state.category?'true':'false')+'" class="category '+(c===state.category?'active':'')+'" data-category="'+esc(c)+'">'+(c==='Autres'?'••• ':'')+esc(c)+'</button>').join('')+'</nav>';
  ...
  <aside class="cart">
    ...
    <div class="cart-foot">
      <div class="cart-tools">
        ...
        <button class="secondary progressive-pay" id="progressive-pay" ${!state.cart.length||!state.online?'disabled':''}>${progressivePaymentActive()?'Continuer par personne':'👤 Par personne'}</button>
      </div>
...
document.querySelector('#nav-sync')?.addEventListener('click',()=>{state.view='sync';updateQueueCount().then(render)});
  ...
  document.querySelectorAll('[data-layout-product]').forEach(b=>b.addEventListener('click',()=>{if(b.disabled)return;openItemConfigurator(b.dataset.layoutProduct)}));

...
export function menuEngineering(state){
  ...
  const portfolio=recipePortfolio(state),items=portfolio.items.map((item,index)=>{const recipe=state?.recipes?.[index]||{},keys=[String(recipe.id||'').toLocaleLowerCase(),String(recipe.name||'').trim().toLocaleLowerCase()].filter(Boolean),sales=keys.map(k=>demand.get(k)).find(Boolean)||{quantity:0,revenue:0},contribution=round(item.margin*sales.quantity,2);return{...item,index,quantity:round(sales.quantity,2),revenue:round(sales.revenue,2),contribution,status:sales.quantity===0?'no_data':item.foodCostPercent>portfolio.warning?'cost_risk':item.margin<=0?'negative':item.foodCostPercent>portfolio.target?'watch':'healthy'}}),sold=items.filter(x=>x.quantity>0),avgQty=sold.length?sold.reduce((sum,x)=>sum+x.quantity,0)/sold.length:0;
  for(const item of items){item.popularity=item.quantity===0?'unknown':item.quantity>=avgQty?'high':'low';item.action=item.status==='cost_risk'?'review_cost':item.status==='negative'?'raise_price':item.popularity==='high'&&item.status==='healthy'?'protect_star':item.popularity==='low'&&item.status==='healthy'?'promote':'monitor'}
  return{target:portfolio.target,warning:portfolio.warning,avgQuantity:round(avgQty,1),items:items.sort((a,b)=>b.contribution-a.contribution||b.quantity-a.quantity)}
}

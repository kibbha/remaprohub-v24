# ReMaPro POS — IP & acquisition-readiness register

**Product:** ReMaPro POS  
**Branch:** `pos/remapro-pos`  
...
ReMaPro-specific POS application code, ordering/payment logic, offline logic, printer integrations written for ReMaPro, tests, documentation, specifications, configuration and product-specific visual assets are treated as proprietary project material unless a file or the third-party register says otherwise.
...
The currently declared direct dependencies are registered as MIT-licensed components, subject to re-verification when versions change.
...
|---|---|---|---|---|
| ReMaPro POS wordmark/logo and product-specific graphics | `app/` and product UI | Created/commissioned for the ReMaPro project under project direction | Proprietary project asset, subject to any underlying third-party element | original source/export, generation/design brief, date, creator/tool, approval commit |
| POS interface layouts, button compositions and ReMaPro-specific visual treatment | `app/`, `src/` | ReMaPro product design | Proprietary project material subject to third-party UI/icon elements, if any | design brief, commits and screenshots |
| UI screenshots / store graphics produced from ReMaPro POS | release/marketing material | Derived from the ReMaPro application | ReMaPro-specific composition; underlying third-party elements retain their licences | source screenshot, release version, editor/source file |
| System fonts | CSS/platform references only | Device/OS fonts; not bundled as ReMaPro font files unless separately recorded | governed by platform/font provider | no font binaries should be committed unless separately licensed |
...
- GitHub repository, POS branch, releases, issues and Actions configuration;
- Supabase/shared backend endpoints and POS synchronization configuration;
- ReMaPro domains/DNS and public website where relevant;
- Google Play Console POS application and package ID `com.remaprohub.pos`;
- Android signing/upload keys and their recovery/rotation procedure;
- payment-terminal/provider integrations and merchant contracts;
- receipt/network/ESC-POS printer integration documentation;
- email/support identities;
...
2. export/backup configuration and backend data where permitted;
3. confirm which provider accounts are transferable and which must be recreated;
4. create buyer/business administrator accounts before removing the seller;
5. inventory signing material and secrets without exposing values in the data room;
...
3. transfer/recreate backend, store, payment and service-provider ownership;
...
3. verify POS sync, store signing, payments, printing and backups;
...
Repository visibility is an operational/security setting, not an IP licence. Before commercial launch, review whether the source repository should remain public; changing visibility must be coordinated with deployment/website workflows so production hosting is not accidentally broken.

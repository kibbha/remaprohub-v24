...
Deno.serve(async(req)=>{
  ...
  try{
    ...
    if(action==="create_order"){
      ...
      const mode=clean(body.serviceType,20);if(!Array.isArray(channel.modes)||!channel.modes.includes(mode))return json({error:"Service mode unavailable"},400);const clientMode=clean(body.clientMode,20)||"web",kioskClient=clientMode==="kiosk";if(kioskClient&&(channel as any).public_config?.kioskEnabled!==true)return json({error:"Kiosk mode unavailable"},403);const allowedPayments=Array.isArray((channel as any).public_config?.paymentMethods)?(channel as any).public_config.paymentMethods.map(String).filter((x:string)=>["counter","card","twint"].includes(x)):["counter"],paymentMethod=clean(body.paymentMethod,20)||"counter";if(!allowedPayments.includes(paymentMethod))return json({error:"Payment method unavailable"},400);let requestedFor:string|null=null;if(mode==="takeaway"&&body.requestedFor){const when=new Date(clean(body.requestedFor,40));if(Number.isNaN(when.getTime())||when.getTime()<Date.now()-5*60000||when.getTime()>Date.now()+31*86400000)return json({error:"Invalid pickup time"},400);requestedFor=when.toISOString()}
      const requested=Array.isArray(body.items)?body.items.slice(0,60):[];if(!requested.length)return json({error:"Order is empty"},400);
      const ipHash=await sha(channel.id+"|"+clientIp(req));const hourAgo=new Date(Date.now()-3600000).toISOString(),{count}=await db.from("direct_orders").select("id",{count:"exact",head:true}).eq("channel_id",channel.id).eq("client_hash",ipHash).gte("created_at",hourAgo),hourlyLimit=kioskClient?300:20;if((count||0)>=hourlyLimit)return json({error:"Too many recent orders"},429);
      ...
      await db.from("direct_order_events").insert({direct_order_id:created.id,event_type:"submitted",details:{serviceType:mode,itemCount:rows.length,paymentMethod,requestedFor,clientMode}});
      return json({ok:true,order:created},201);
    }
    if(action==="status"){
      const id=clean(body.orderId,64),ref=clean(body.reference,40);if(!validUuid(id)||!ref)return json({error:"Order reference required"},400);const {data}=await db.from("direct_orders").select("id,public_reference,status,payment_status,payment_method,total,currency,requested_for,created_at,accepted_at,completed_at").eq("id",id).eq("channel_id",channel.id).eq("public_reference",ref).maybeSingle();return data?json({ok:true,order:data}):json({error:"Order not found"},404)
    }

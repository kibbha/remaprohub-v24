...
const stableKeySql=fs.readFileSync(new URL('../supabase/migrations/20260923151341_pos_availability_stable_keys.sql',import.meta.url),'utf8');
...
assert.ok(edge.includes('const resetRequested='));
assert.ok(edge.includes('quantityChanged||resetRequested?cfg.manualQuantity'));
assert.ok(edge.includes('Date.parse(cfg.resetAt)>Date.parse(existing.updated_at)'));
...
assert.ok(stableKeySql.includes("value->>'availabilityKey'"));
assert.ok(stableKeySql.includes("value->>'kind'='remapro_availability'"));
assert.ok(stableKeySql.includes("b->>'id'=substring(v_key from 8)"));
...
assert.ok(posBridge.includes("resetAt:String(item?.posAvailabilityResetAt||'')"));

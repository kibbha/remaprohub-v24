...
assert.match(admin,/MANAGER_ROLES/);
assert.match(admin,/RESTAURANT_ADMIN_ROLES/);
assert.match(admin,/STAFF_PERMISSIONS/);
assert.match(admin,/trialActive[\s\S]*?7 \* 86400000/,'fallback trial must remain bounded to seven days');
assert.match(admin,/status === "active" && planCode === "multi"/);

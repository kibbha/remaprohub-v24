...
for(const token of ['hub-classic-home','hub-home-intro','hub-ca-banner','hub-home-section','hub-category-grid'])
...
const dedicatedForms=new Set(['settingsForm','securitySettingsForm','securityLoginForm','securitySignupForm','developerLoginForm','developerAccessForm','memberAccessForm','billingPurchaseForm','cloudForm','cloudLoginForm','aiForm','visionStockForm','invoiceReviewForm','deliveryScanReviewForm','restaurantForm','managerForm','staffAccessForm','directChannelForm','posTableAddForm','posOperatorAddForm','posPrinterAddForm','posTerminalAddForm','posProviderAddForm','supportTicketForm','supportReplyForm']);
...
for(const formId of ['productForm','tempForm','stockForm','recipeForm','auditForm','goalForm']){
  assert.match(app,new RegExp(`form\\('${formId}',f=>\\{if\\(!recordValidated`),`${formId} must use recordValidated`);
}
assert.match(app,/form\('advancedReservationForm',[\s\S]{0,1200}recordAdvancedReservation\(state,/,'advanced reservations must use the dedicated validated store path');
assert.match(app,/form\('loyaltyTransactionForm',[\s\S]{0,500}loyaltyTransaction\(state,/,'loyalty transactions must use the dedicated validated store path');
...
assert.match(app,/id="advancedReservationForm"[\s\S]{0,700}name="covers"[^>]*min="1"[^>]*step="1"/);

...
for(const invalid of ['not json',JSON.stringify({version:26,state:{}}),JSON.stringify({version:27,state:{...JSON.parse(backup).state,docs:{}}})]){const before=JSON.stringify(state);assert.equal(importData(state,invalid),false);assert.equal(JSON.stringify(state),before)}
console.log('Backup restore and rejection OK');

...
try{writeFileSync(file,'android { defaultConfig {\n  versionCode 1\n  versionName "1.0"\n}}');execFileSync(process.execPath,['scripts/stamp-android-version.mjs',file],{env:{...process.env,GITHUB_RUN_NUMBER:'42'}});const gradle=readFileSync(file,'utf8');assert.match(gradle,/versionCode 270042/);assert.match(gradle,/versionName "27\.0\.0"/)}finally{rmSync(dir,{recursive:true,force:true})}

...
try{
  writeFileSync(file,'android { defaultConfig {\n  versionCode 1\n  versionName "1.0"\n}}');
  execFileSync(process.execPath,['scripts/stamp-android-version.mjs',file],{env:{...process.env,GITHUB_RUN_NUMBER:'42'}});
  const gradle=readFileSync(file,'utf8');
  assert.match(gradle,/versionCode 270042/);
  assert.ok(gradle.includes(`versionName "${pkg.version}"`));
}finally{rmSync(dir,{recursive:true,force:true})}
const workflow=readFileSync('.github/workflows/android.yml','utf8');
assert.ok(workflow.indexOf('Stamp Android version')<workflow.indexOf('npx cap sync android'));

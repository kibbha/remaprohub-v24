...
import {calculateSwissPayroll,updatePayrollSettings,ccntMinimum} from '../src/store.js';
...
assert.equal(r.netSalary,4680);
assert.ok(r.warnings.includes('TODO_VERIFIER_LPP'));
assert.ok(r.warnings.includes('TODO_VERIFIER_ACCIDENT'));
...
assert.equal(r.warnings.includes('TODO_VERIFIER_LPP'),false);
assert.equal(r.warnings.includes('TODO_VERIFIER_ACCIDENT'),false);
assert.equal(updatePayrollSettings(state,{acEmployee:-1}),false);
console.log('Swiss payroll configuration and calculations OK');

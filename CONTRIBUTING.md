...
ReMaPro POS is proprietary software. Contributions are accepted only when the project owner can document a clear right to use, modify, sublicense and transfer the contribution.
...
Commits from external contributors should include a Signed-off-by trailer. The sign-off records traceability only; it does not replace any contract or IP assignment required by the project owner.
...
3. verify licence/security posture;
4. run `npm run ip:check`.

If an asset changes:
1. update `docs/ip-compliance/ASSET_REGISTER.csv`;
2. retain confidential evidence outside Git;
